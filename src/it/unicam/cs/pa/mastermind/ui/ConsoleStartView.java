...
public class ConsoleStartView implements StartView {
			...
			while (toContinue) {
				...
				if (!keepSettings) {
					...
					if (this.askNewSettings()) {
						attempts = askNewAttempts(lowTresholdAttempts);
						sequenceLength = askNewlength(lowTresholdLength, highTresholdLength);
					}
				}
				...
				currentGame = new SingleGame(this.sequenceLength, this.attempts, this.intView, currentBreaker,
						currentMaker);
				currentGame.start();
				NewGameStats newStats = this.getNewSettings();
	...
	private NewGameStats getNewSettings() {
		int intInput = 0;
		try {
	 ...
	 * @throws IOException
	 */
	private void setupNewPlayers() throws NumberFormatException, IOException {
	 ...
	 */
	private boolean askNewSettings() throws IOException {
		String strInput = "";
	 ...
	 * @throws IOException se il valore inserito non è un valore numerico
	 */
	private int askNewAttempts(int lowTres) throws IOException {
		int attempts = 0;
		do {
			try {
				System.out
	 ...
	 * @throws IOException se il valore inserito non è un valore numerico
	 */
	private int askNewlength(int lowTres, int highTres) throws IOException {
		int length = 0;
		do {
			try {
